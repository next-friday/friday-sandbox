...
- A genuinely free length a consumer sets per instance — a responsive grid's minimum track width — is the one length literal allowed in component CSS, and only as a **component-local token with a sensible default**: `.fri-grid { --grid-min: 16rem }`, consumed via `var(--grid-min)`. The token is itself the overridable knob; a consumer retunes it at the element (inline `style={{ "--grid-min": … }}`, or a selector that targets `.fri-grid`).
