...
- Defaults belong in `defaultVariants` (tv) or in tooling, not in CSS base rules.
- Raw HTML use (`<button class="fri-button">` alone) is fine — but the base must be either fully neutral (works alone, looks unstyled) OR carry an explicit duplicate of the default class's tokens (so raw use still resolves). Choose one, document it.
