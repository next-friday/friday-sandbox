...
NEW_CONTENT=$(echo "$INPUT" | jq -r '[.tool_input.content, .tool_input.new_string, (.tool_input.edits[]?.new_string)] | map(select(. != null)) | join("\n")')
...
if [[ "$FILE_PATH" =~ ^(.*/)?(packages|apps)/.*\.(ts|tsx|mjs|cjs|css)$ ]]; then
  if echo "$NEW_CONTENT" | grep -Eq 'eslint-disable|@ts-(expect-error|ignore|nocheck)|stylelint-disable|prettier-ignore|biome-ignore'; then
