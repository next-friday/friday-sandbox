...
5. **Gates via the hooks.** Pre-commit runs them on staged files; pre-push runs `format:check`, `sort:check`, `lint:prose`, `lint:symmetry`, then the affected-package tasks (`lint`, `typecheck`, `build`, `build:storybook`, `test`), the graph gates (`knip`, `depcruise`), plus `audit`. Do not run whole-repo gates by hand; never `--no-verify` or disable a rule — fix the root cause.
