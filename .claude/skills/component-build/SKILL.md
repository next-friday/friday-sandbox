...
5. **Stories in `<name>.stories.tsx` ARE the tests** (Vitest in Chromium via Playwright, `storybook/test`). Follow `button.stories.tsx`: full `argTypes`; a showcase story per variant axis (every value, laid out with `Flex`) and per state — render-only visual coverage, no `play`. For interactive bases, add `play` on `Default` (interaction — `userEvent.tab()` → `toHaveFocus`, keyboard → a `fn()` handler) and `BaseClassDefault` (`getComputedStyle` — the base class renders usably with no modifiers); for display/layout bases (text, flex, grid, separator, spinner), skip only the `userEvent` behavior plays — a presence (`toBeInTheDocument`) or `getComputedStyle` assertion on `Default` / `BaseClassDefault` is still welcome (the `native` scaffold ships both, though a structural base may instead prove the base class through `Default` / `PlainHtml` alone, as `text` does). The scaffold's `BaseClassDefault` asserts `display` differs — valid for the scaffold's `block` default, but if your base keeps the element inline (`kbd`, `code`), switch to the font-size-container probe below. The `BaseClassDefault` `getComputedStyle` assertion must target a property that **collapses or changes when the base default is removed** (separator's `height > 0` holds because a bare `.fri-separator` collapses to 0) — never one that inherits a nonzero browser default (`fontSize > 0` passes even with the rule deleted: a vacuous test). For typography, render the base-class element beside an unclassed sibling in a container whose font-size differs from the default and assert the two differ. Keep these probe stories lint-clean — lift any inline `style={{…}}` object to a module const and give every probe element a `data-testid`, or a bare `<span>`/`<div>` or inline-object prop trips `next-friday/no-ghost-wrapper` and `jsx-simple-properties`. The `native` scaffold already emits the class-only `PlainHtml` story (the `fri-` classes work on a plain element).
...
- [ ] `<name>.css` mirrors `.variants.ts` 1:1 — self-checked by reading the two, enforced by the pre-commit `lint:symmetry` hook — via the ladder + `:where()` default + ramp-multiplier pattern; no orphan class either side; no hardcoded color (use the `--fri-<role>` ladder).
...
- **Self-check the mirror first** by reading `<name>.variants.ts` against `<name>.css`: every `fri-<name>-<value>` on one side has its rule on the other, no orphan either way, every value distinct. Do **not** run `pnpm lint:symmetry` by hand — `CLAUDE.md` reserves it and the other whole-repo gates for the pre-commit/pre-push hooks, which enforce the mirror deterministically at commit. Fix every orphan you spot before the judgment pass.
...
- Stories are the gate. Verify the new component's stories at the build station with the scoped single-file run `pnpm --filter @friday-sandbox/react exec vitest run <name>` (one story file — blessed in `CLAUDE.md` Commands, not a whole-repo gate), so a broken `play` or a vacuous assertion surfaces here, not at ship. Let the hooks run the full suite; never run whole-repo gates by hand.
...
| --------- | -------------------- | ---------------------------- | ------------------------ | -------------------------------- |
| `<name>`  | 5 surfaces + barrels | `<roles>×<variants>×<sizes>` | `<count>` green (scoped) | hooks: `lint:symmetry` + stories |
...
- A raw Tailwind numeric scale (`ms-0.5`, `gap-1`) where a `--fri-spacing-*` token exists (`ms-2xs`) → use the design-token ladder, not the framework's default scale.
...
- A `BaseClassDefault` assertion on a property with a nonzero inherited default (`fontSize > 0`) → vacuous; assert a property that collapses when the base default is removed.
