...
Count the fixes. If a third fix reveals new coupling somewhere else — a style chased across `.variants.ts` → `.css` → token → barrel, each fix surfacing a fresh break — STOP: the component's variant or token shape is wrong. Escalate to `component-design`; do not attempt fix #4.
