...
- Invented a token, a raw Tailwind numeric scale (`ms-0.5`, `gap-1`), or a `gap-(--fri-spacing-*)` var form where a **semantic alias** exists (`gap-sm`, `p-md` map to `--fri-spacing-*` in `theme/tailwind.css`) → use the alias, never hardcode a color.
