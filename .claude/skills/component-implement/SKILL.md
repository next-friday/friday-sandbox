...
2. **Build the component to its contract** on that branch — see [Building the component](#building-the-component). Self-check the mirror, run the scoped story file, then the two-axis sub-agent audit; iterate until the build contract holds. If the spec shifts here (a renamed prop, a dropped axis), loop back through `component-blueprint` and return — cheap while local, costly once the PR is open.
...
4. **Gates via the hooks.** Pre-commit runs them on staged files; pre-push runs `format:check`, `sort:check`, `lint:symmetry`, then the affected-package tasks (`lint`, `typecheck`, `build`, `build:storybook`, `test`), the graph gates (`knip`, `depcruise`), plus `audit`. Do not run whole-repo gates by hand; never `--no-verify` or disable a rule — fix the root cause. No completion claim without fresh evidence: the hooks' output is the proof, never "should pass".
5. **Open the PR.** Title is Conventional Commits `type(scope): subject` — type from the repo enum (`feat fix docs refactor perf chore build ci style test …`), scope the area (`react`, `styles`, `docs`, `tooling`), ≤50 chars, lowercase imperative, no body or footer. Put `Closes #<n>` in the BODY, never the title. Fill the PR template; add a screenshot for visual changes.
...
2. **Make it the real component.** Swap the scaffold's placeholder element for the real primitive and widen the props:
   ...
   - **Import from the package root** (`react-aria-components`), matching `button`/`text`/`separator` — always. A per-component subpath (`react-aria-components/<Part>`) diverges from the other bases and forces an `optimizeDeps.include` entry in `.storybook/main.ts` (without it Storybook/Vitest re-optimize mid-run and the import fails), so don't reach for it.
...
5. **Stories in `<name>.stories.tsx` ARE the tests** (Vitest in Chromium via Playwright, `storybook/test`). Follow `button.stories.tsx`:
   - **Showcase coverage**: full `argTypes`; one story per variant axis (every value, laid out with `Flex`) and one per state — render-only, no `play`.
   - **Interactive bases**: add `play` on `Default` (interaction — `userEvent.tab()` → `toHaveFocus`, keyboard → a `fn()` handler) and on `BaseClassDefault` (`getComputedStyle` — the base class renders usably with no modifiers).
   - **Display/layout bases** (text, flex, grid, separator, spinner): skip only the `userEvent` behavior plays — a presence (`toBeInTheDocument`) or `getComputedStyle` assertion on `Default` / `BaseClassDefault` is still welcome (the `native` scaffold ships both, though a structural base may instead prove the base class through `Default` / `PlainHtml` alone, as `text` does).
   - **`BaseClassDefault` must not be vacuous**: assert a property that **collapses or changes when the base default is removed** (separator's `height > 0` holds because a bare `.fri-separator` collapses to 0) — never one that inherits a nonzero browser default (`fontSize > 0` passes even with the rule deleted). The scaffold asserts `display` differs (valid for its `block` default); if your base stays inline (`kbd`, `code`), switch to the typography probe: render the base-class element beside an unclassed sibling in a container whose font-size differs from the default and assert the two differ.
...
- [ ] `<name>.css` mirrors `.variants.ts` 1:1 — self-checked by reading the two, enforced by the pre-commit `lint:symmetry` hook — via the ladder + `:where()` default + ramp-multiplier pattern; no orphan class either side; no hardcoded color.
- [ ] States wired via react-aria data attrs and repo utilities (`focus-ring`, `status-disabled`, `transition-base`).
- [ ] Stories cover every variant value and every state; interactive bases add `play` on `Default` + `BaseClassDefault`, display/layout bases stay render-only; plus a class-only story; all pass (scoped run).
- [ ] mdx on the STYLE.md spine; `meta.json` carries `<name>`. Compound exports / `.types.ts` / `.namespace.ts` added if needed.
...
- **Self-check the mirror first** by reading `<name>.variants.ts` against `<name>.css`: every `fri-<name>-<value>` on one side has its rule on the other, no orphan either way, every value distinct. Do **not** run `pnpm lint:symmetry` by hand — `CLAUDE.md` reserves it and the other whole-repo gates for the pre-commit/pre-push hooks, which enforce the mirror deterministically at commit.
...
- Invented a token or a raw Tailwind numeric scale (`ms-0.5`, `gap-1`) where a `--fri-*` token exists → use the design-token ladder, never hardcode a color.
...
- A variant value or state with no story, an interactive base missing `play`, or a `BaseClassDefault` asserting a nonzero inherited default (`fontSize > 0`) → coverage gap / vacuous test.
