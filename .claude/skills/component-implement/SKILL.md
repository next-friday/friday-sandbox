...
4. **Mirror in `packages/styles/src/components/<name>.css`** — a sibling package to the `<name>.styles.ts` it mirrors, linked only by the `fri-<name>` class name — following `button.css`. Style every rule through `@apply`; a property Tailwind has no utility for becomes a named `@utility` in `layers/utilities.css` (see `status-disabled`), never a raw declaration (the apply-only gate rejects it, so recording one would contradict the build). `--*` custom-property defs (the ramp multiplier) are var machinery and stay:
   ...
   - one `fri-<name>-<color>` class per role, wiring the ladder into local slots (`--fri-<role>` plus its `-foreground`, `-secondary`, `-secondary-foreground` seeds and the derived `-hover`/`-pressed`, `-secondary-hover`/`-secondary-pressed`, `-border`, `-outline-border`, `-tint-hover`/`-tint-pressed` steps — the full set is in `button.css`).
