...
4. **Mirror in `packages/styles/src/components/<name>.css`** — a sibling package to the `<name>.styles.ts` it mirrors, linked only by the `fri-<name>` class name — following `button.css`:
