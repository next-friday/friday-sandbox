...
Compare on three lenses: leverage per prop; locality (does the decision live in one spec field, or does it ripple across `variants`, `parts`, and `demo`); seam placement (react-aria vs radix vs native). Recommend one, or hybridize — "the header from B with the sidebar from C" is often the real design.
