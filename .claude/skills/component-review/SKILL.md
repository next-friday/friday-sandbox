...
3. **A sub-issue only for work you will NOT fix in this PR.** A finding you fix now — every style-guide nit, every quick correctness fix — needs no sub-issue: the PR thread reply plus the fix commit are the trace. Open a sub-issue (linked to the parent) only for a real finding you are deferring or that is cross-cutting; the ship entry-authorization already covers it, so don't open a fresh chat prompt per finding. Write a deferred sub-issue behaviorally — current vs desired, a testable acceptance criterion, and an out-of-scope line so the fix isn't gold-plated — referencing interfaces by type, not a file path that goes stale.
