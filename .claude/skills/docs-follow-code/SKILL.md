...
3. **Walk the full target table** — one grep pattern cannot name every class; check each row deliberately:
   ...
   | Generators        | `packages/component-generator/src/emit/*` — a stale emitter regenerates stale output forever; fix the emitter AND its emitted output in the same pass |
...
| Swept generated `.mdx` but not `emit/mdx.ts` → the next generator run regenerated the stale links                                                                                                                                                                       | step 3 — the emitter is canonical; sweep it first                                                                                                                         |
