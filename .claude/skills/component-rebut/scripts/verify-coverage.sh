#!/usr/bin/env bash
# verify-coverage.sh <pr>: re-query a pull request after a review round and assert
# that every inline bot finding received a reply from the triage account, so a
...
# Triage identity: under a GitHub App token `gh api user` returns 403, so accept
# the app's `slug[bot]` login via TRIAGE_LOGIN and fall back to the authenticated
# user only for a personal token.
me="${TRIAGE_LOGIN:-}"
if [ -z "$me" ]; then
