...
- [ ] Looped until the AI round was machine-confirmed clean (`verify-coverage` N/N + `ci-status` green over the reported non-gated checks), then handed to the human's gate; never merged without the human.

## Verify

- Machine-confirm the round, don't declare it clean by eye: `"${CLAUDE_SKILL_DIR}/scripts/verify-coverage.sh" <pr>` reads `answered N / N` and `"${CLAUDE_SKILL_DIR}/scripts/ci-status.sh" <pr>` reads `ci: green` (`core` scope, or `all` when no gated check is present).
...
- Bot identity differs by API surface — GraphQL `author.login` omits the `[bot]` suffix REST `user.login` carries — and both bots post placeholders before the real round; `wait-for-round.sh` is the single home for that detection (done = a review submitted at the current head SHA; CodeRabbit's walkthrough comment flipping to "Currently processing" is the re-review signal and always wins).
