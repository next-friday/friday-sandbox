...
9. **Hand to the human checkpoint.** Confirm the round is clean by machine, not by eye: `bash "${CLAUDE_SKILL_DIR}/scripts/verify-coverage.sh" <pr>` must report `answered N / N` (every bot finding has a reply) and `bash "${CLAUDE_SKILL_DIR}/scripts/ci-status.sh" <pr>` must read `ci: green` (the verdict scopes to the **required checks** when the ruleset defines them, else to the **core checks** — a secret- or human-gated check like pkg-pr-new or Chromatic's UI Review is reported as informational and doesn't block). Then hand the human the link to the round-summary comment (step 7) for their final review on the PR (the merge gate). The human reads the threads and merges; you never merge. If the human requests changes, they re-enter here as a new round.
...
- Chromatic (`CHROMATIC_PROJECT_TOKEN`) can show red when its secret isn't configured (`.github/workflows/chromatic.yml`); whether any check blocks merge is branch-protection config — `ci-status.sh` verdicts from `gh pr checks --required` when the ruleset defines required checks, else from the core set with the known secret-/human-gated checks (pkg-pr-new, Chromatic UI Review/Tests, Sonar) split out as informational, so an optional red can't fake a failing round.
