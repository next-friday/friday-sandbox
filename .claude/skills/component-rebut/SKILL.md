...
7. **Post the round summary on the PR — as tables, for the human reader.** After the thread replies, post one top-level comment (`gh pr comment <pr> --body-file …`), opening with the same disclosure blockquote, then an H2 naming the round and the reviewed head sha. The body is two tables, never prose paragraphs: a findings table `| Finding | Verdict | Evidence |` — one row per finding, Verdict `Fixed` (Evidence names the commit sha, or the sub-issue if deferred) or `Rebutted` (Evidence is the one-line reason) — and a reviewer table `| Reviewer | This round |` with one row per expected reviewer, including any that never posted or declined the head.
...
- Post exactly one top-level round-summary comment per round, in addition to the per-thread replies; its body is the step-7 tables, never prose paragraphs.
