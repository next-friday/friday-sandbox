...
7. **Post the round summary on the PR.** After the thread replies, post one top-level comment (`gh pr comment <pr> --body-file …`), opening with the same disclosure blockquote: the round number, what was fixed (each with commit sha), what was rebutted (one-line reason each), and any reviewer that never posted.
...
- Verify every finding against this codebase before filing — no performative agreement, no reflexive dismissal.
...
- Post exactly one top-level round-summary comment per round, in addition to the per-thread replies.
