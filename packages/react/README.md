...
Welcome! Friday UI is a kit of polished, ready-to-use pieces, like buttons, layout primitives, and more, that you can drop into your project today. Think of it as a box of Lego pieces for the web: each piece is already painted, already fits, and you just choose where it goes.
...
Friday UI ships with a little playground called **Storybook**, a website that shows every piece in every state, such as normal, hovered, disabled, and loading, so you can play with it before adding it to your project. The playground is built from `packages/react` and published via [`vercel.json`](https://github.com/next-friday/friday-sandbox/blob/main/vercel.json).
