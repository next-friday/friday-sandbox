...
//
// Run: node --experimental-strip-types scripts/check-component-symmetry.mts
...
const classesFrom = (text: string, name: string): string[] => {
  const matches = text.match(new RegExp(`fri-${name}(?:-[a-z0-9]+)*`, "g"));
  return matches ?? [];
};
...
const components = readdirSync(REACT_BASES, { withFileTypes: true })
...
for (const name of components) {
  ...
  if (!existsSync(variantsPath)) continue; // dir without a variants surface
