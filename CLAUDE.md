...
- **Generate components; do not hand-roll.** Scaffold a base component with `pnpm gen component` (Turborepo `turbo gen`; the generator and its Handlebars templates live in `turbo/generators/`, authored in TypeScript). It writes the component files and wires every export barrel and the styles `@import`. Do not hand-create those files.
