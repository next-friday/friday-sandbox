...
- **Generate components; do not hand-roll.** A base component is designed as a transient `ComponentSpec` (`packages/component-generator/src/component-spec.ts`) and generated by running the engine's CLI on it (`node --experimental-strip-types packages/component-generator/src/cli.ts <spec>`) — never hand-create the emitted files or add a second changeset. The `component-*` skill suite (`.claude/skills/component-{blueprint,implement,rebut}/`) drives the issue-driven lifecycle — `component-blueprint` authors the spec, `component-implement` runs the engine and fills the residual — each writing to the shared tracker only with explicit per-artifact authorization. Full engine behavior and the blueprint → implement → rebut workflow: [`CONTRIBUTING.md`](CONTRIBUTING.md).
- **Skill docs mirror the code they describe.** The `component-*` skills reference the engine (`packages/component-generator/`), the base-component patterns, and the token ladder by concrete path, token name, and emitted output. Change one of those and the skill rots silently — update the skill in the **same change**, the way `src` ↔ `exports` stay aligned. When a skill and the tree disagree, the tree is right and the skill is the bug.
