...
- **TypeScript only, never `.js`.** Source and scripts are `.ts`/`.tsx`, run with `node --experimental-strip-types` (Node's native type-stripping, not `tsx`/`ts-node`); a config a tool loads only as ESM is `.mjs`. No exceptions — even the symmetry gate (`scripts/check-component-symmetry.ts`) is TypeScript, reading real ASTs (the TypeScript compiler API for `.ts`/`.tsx`, postcss for CSS) instead of a text scan.
