...
- **TypeScript only, never `.js`.** Source and scripts are `.ts`/`.tsx`, run with `node --experimental-strip-types` (Node's native type-stripping, not `tsx`/`ts-node`); a config a tool loads only as ESM is `.mjs`. The one exception is the text-scan gate `scripts/check-component-symmetry.sh`, a shell script — its logic is grep/awk over literal conventions, not a TS/CSS AST.
- Conventions auto-load from [`.claude/rules/`](.claude/rules) — the global rules every session, the path-scoped rules (architecture, prose style, markdown style, comments) when you touch a matching file. Process lives in [`CONTRIBUTING.md`](CONTRIBUTING.md).
