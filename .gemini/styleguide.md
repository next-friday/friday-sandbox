...
- Story copy is consumer-facing: flag internal class names such as `fri-button-*` and `fri-flex-*`, library names such as `class-variance-authority` and `react-aria`, engine math such as `calc(var(--fri-spacing-xs) * var(--_button-n))`, or file paths. Symmetric story shape across files.
