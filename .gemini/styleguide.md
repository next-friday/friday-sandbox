...
- Size tokens scoped to `action`, `field`, or `box`, never a literal component name; corner radius is Tailwind native (`rounded-*`), not a token. The `themes/` CSS is hand-authored token values; keep the `@theme` map (`shared/theme.css`) and `@property` registrations (`shared/registered.css`) in sync with the token names.
