...
- Size tokens scoped to `action`, `field`, or `box`, never a literal component name; corner radius is Tailwind native (`rounded-*`), not a token. The `src/theme/` CSS is hand-authored token values; keep the `@theme` map (`tailwind.css`) and `@property` registrations (`registered.css`) in sync with the token names.
